...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private final List<String> publicNotificationPaths = Arrays.asList(
        "/notifications/*",
        "/api/notifications/*"
    );
    ...
    private boolean isPublicPath(String path, String method) {
        ...
        if (publicNotificationPaths.stream().anyMatch(pattern -> pathMatcher.match(pattern, path))) {
            log.debug("通知相关路径需要认证: {}", path);
            return false;
        }
    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            if (userEmail != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                ...
                if (jwtService.isTokenValid(jwt, userDetails)) {
                    ...
                    boolean isAdmin = userDetails.getAuthorities().stream()
                            .anyMatch(a -> a.getAuthority().equals("管理员"));
            ...
            response.setContentType("application/json;charset=UTF-8");
            response.getWriter().write("{\"message\":\"Token验证失败\"}");
            return;
        }
